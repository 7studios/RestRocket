...
+ (ARCShadowStyle*)styleWithColor:(UIColor*)color blur:(CGFloat)blur offset:(CGSize)offset next:(ARCStyle*)next {
    
    ARCShadowStyle* style = [[self alloc] initWithNext:next];
    style.color = color;
    style.blur = blur;
    style.offset = offset;
...
- (void)draw:(ARCStyleContext*)context {
  ...
  context.frame = ARCRectInset(context.frame, inset);
  context.contentFrame = ARCRectInset(context.contentFrame, inset);
